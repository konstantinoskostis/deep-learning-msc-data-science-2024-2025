...
class ESC50Loader:
    ...
    def load(self):
        ...
        train_df['audio_path'] = train_df['filename'].apply(lambda x: os.path.join(self.path, ESC_50_AUDIO_PATH, x))
        val_df['audio_path'] = val_df['filename'].apply(lambda x: os.path.join(self.path, ESC_50_AUDIO_PATH, x))
        test_df['audio_path'] = test_df['filename'].apply(lambda x: os.path.join(self.path, ESC_50_AUDIO_PATH, x))
